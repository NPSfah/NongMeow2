...
export class LocalCommandParser {
  parse(text: string): LocalCommandResult | undefined {
    ...
    if (this.matches(command, ['help', 'h', '?', 'ช่วยเหลือ', 'คำสั่ง', 'สอนใช้'])) {
      return {
        ...
        text: [
          ...
          '- แก้งาน... เช่น "แก้งานประชุมพรุ่งนี้เป็นบ่ายสอง"',
        ].join('\n'),
      };
    }
    ...
    if (this.matches(command, ['edit help', 'วิธีแก้', 'แก้ยังไง'])) {
      return {
        intent: 'direct_reply',
        text: 'พิมพ์แก้ตามด้วยงานและสิ่งที่อยากเปลี่ยน เช่น "แก้งานประชุมพรุ่งนี้เป็นบ่ายสอง" หรือ "เปลี่ยนปลุกพรุ่งนี้เป็น 9 โมง"',
      };
    }
